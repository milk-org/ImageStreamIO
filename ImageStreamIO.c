...
errno_t ImageStreamIO_set_printError(errno_t (*new_printError)(
    const char *, const char *, int, errno_t, char *)) {
...
errno_t ImageStreamIO_write_process_log(char *msg) {
  ...
  if (fplog != NULL) {
    ...
    fprintf(fplog, "%04d%02d%02dT%02d%02d%02d.%09ld %s\n",
            ...
            uttime->tm_hour, uttime->tm_min, uttime->tm_sec, tnow.tv_nsec, msg);
...
errno_t ImageStreamIO_readBufferAt(const IMAGE *image,
  ...
  *buffer =
      (void *)(image->array.UI8 + slice_index * frame_size * size_element);
...
errno_t ImageStreamIO_filename(char *file_name, size_t ssz,
                               const char *im_name) {
...
int ImageStreamIO_bitpix(uint8_t datatype) {
  switch (datatype) {
    ...
    default:
      ...
      return -1;  // This is an in-band error code, must be unique from valid
                  // BITPIX values.
  }
}
  ...
  if (image->md->location == -1) {
    if (image->md->shared == 1) {
      ...
      if (image->array.raw == NULL) {
        ...
        exit(
            EXIT_FAILURE);  ///\todo Is this really an exit or should we return?
      }
    }
    ...
    ImageStreamIO_printERROR(IMAGESTREAMIO_NOTIMPL,
                             "unsupported location, milk needs to be compiled "
                             "with -DUSE_CUDA=ON");  ///\todo should this return
                                                     /// an error?
#endif
  }
...
errno_t ImageStreamIO_createIm_gpu(
    IMAGE *image, const char *name, long naxis, uint32_t *size,
    uint8_t datatype,
    int8_t location,  // -1: CPU RAM, 0+ : GPU
    int shared, int NBsem, int NBkw, uint64_t imagetype,
  ...
  if (((imagetype & 0xF000F) == CIRCULAR_BUFFER) && (naxis != 3)) {
    ImageStreamIO_printERROR(IMAGESTREAMIO_INVALIDARG,
  ...
  if (shared == 1) {
...

    {
      ...
      image->md->inode = file_stat.st_ino;  // inode now contains inode number
                                            // of the file with descriptor fd
    }
  ...
  if (shared == 1) {
    ...
    for (int proctraceindex = 0; proctraceindex < NBproctrace;
         proctraceindex++) {
  ...
  // DEBUG_TRACEPOINT_LOG("%s %d NBsem = %d", __FILE__, __LINE__,
  // image->md->sem);
...
void *ImageStreamIO_get_image_d_ptr(IMAGE *image) {
  ...
  if (image->md->location >= 0) {
    ...
    ImageStreamIO_printERROR(IMAGESTREAMIO_NOTIMPL,
                             "Error calling ImageStreamIO_get_image_d_ptr(), "
                             "wrong location");  ///\todo should this return a
                                                 /// NULL?
  }
  return d_ptr;
}
...
errno_t ImageStreamIO_read_sharedmem_image_toIMAGE(const char *name,
  ...
  // printf("File %s size: %zd\n", SM_fname, file_stat.st_size); fflush(stdout);
  // //TEST
  ...
  if (strcmp(image->md->version, IMAGESTRUCT_VERSION)) {
    char errmsg[200];
    sprintf(errmsg,
            "Stream %s corrupted, or incompatible version. Should be %s", name,
            IMAGESTRUCT_VERSION);
  ...
  */
  ...
  if ((image->md->imagetype & 0xF000F) == (CIRCULAR_BUFFER | ZAXIS_TEMPORAL)) {
  ...
  while (sOK == 1) {
    snprintf(sname, sizeof(sname), "%s.%s_sem%02ld", shmdirname,
             image->md->name, snb);
    sem_t *stest;
    umask(0);
  ...
  for (s = 0; s < image->md->sem; s++) {
    ...
    if ((image->semptr[s] = sem_open(sname, 0, FILEMODE, 0)) == SEM_FAILED) {
      // printf("ERROR: could not open semaphore %s -> (re-)CREATING
      // semaphore\n",
      //        sname);
...
int ImageStreamIO_createsem(IMAGE *image, long NBsem) {
  ...
  for (s = 0; s < NBsem; s++) {
    ...
    snprintf(sname, sizeof(sname), "%s.%s_sem%02ld", shmdirname,
             image->md->name, s);
    umask(0);
    if ((image->semptr[s] = sem_open(sname, O_CREAT, FILEMODE, 0)) ==
        SEM_FAILED) {
      ImageStreamIO_printERROR(IMAGESTREAMIO_SEMINIT,
...
long ImageStreamIO_UpdateIm(IMAGE *image) {
  if (image->md->shared == 1) {
    ...
    if (image->md->CBsize > 0) {
      ...
      // destination pointer
      void *destptr;
      destptr = image->CBimdata + image->md->imdatamemsize * CBindexWrite;

      memcpy(destptr, image->array.raw, image->md->imdatamemsize);
